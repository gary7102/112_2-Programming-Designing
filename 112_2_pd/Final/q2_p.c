...
int circlePeriemter(circle_t *c){return c->radius * 2 * PI;}
int circleArea(circle_t *c){return c->radius * c->radius * PI;}
int rectanglePerimeter(rectangle_t *r){return 2*(r->width + r->height);}
...
void sortShape(shape_t *shapes[], int num){
    ...
    for(int i=0; i<num; i++){
        for(int j=0; j<num-i-1; j++){
            if(shapes[j]->perimeter(shapes[j]) > shapes[j+1]->perimeter(shapes[j]) || 
            ((shapes[j]->perimeter(shapes[j]) == shapes[j+1]->perimeter(shapes[j])) && (shapes[j]->area(shapes[j]) > shapes[j+1]->area(shapes[j])))){
                shape_t *temp = shapes[j];
                shapes[j] = shapes[j+1];
                shapes[j+1] = temp;
            }
